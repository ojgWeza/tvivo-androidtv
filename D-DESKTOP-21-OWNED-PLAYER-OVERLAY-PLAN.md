...
The original render-API route was one OpenGL context that is both current and owned by the actual Skiko layer which Compose Desktop uses to paint this `ComposeWindow`. mpv would render first into an off-screen OpenGL FBO owned by the prototype; Skia would then draw that texture as the video background in the same Skiko frame; ordinary Compose would draw the controller above it. That route is now closed by the evidence in 0.3.2-0.3.3. The historical design deliberately had no `SwingPanel`, `Canvas`, child HWND, second GL window, readback to CPU, or attempt to place Compose above a heavyweight component; the alternate path below revisits only the second-window boundary.
...
This result proves the Compose/Skiko hook and ordering only. It does **not** prove libmpv ABI compatibility, FBO creation, texture wrapping, context loss, resize behavior, or lifecycle teardown. No `MpvPlayer`, `DesktopShell`, `wid`, OSC, or production behavior was changed by this spike. The GL binding/prototype is now closed by 0.3.2-0.3.3; the next work item is the alternate compositing investigation in 0.1.2.

#### 0.1.2 Alternate compositing path investigation (2026-09-17)

**Decision:** do not retry `MPV_RENDER_API_TYPE_OPENGL`, change the resolver again, search for another Skiko GL hook, or treat ANGLE as a new backend. The evidence in 0.3.3 shows a current GL 4.6 context, correct native init-param memory, a live callback pointer, and zero callback invocations before mpv reports `Can't load OpenGL functions` / `OpenGL not initialized`. This is an mpv render-context creation failure earlier than caller-side GL symbol resolution. A native debugger with mpv symbols or mpv-team guidance could explain it, but it is not a sound next implementation dependency.

The candidates were evaluated against the hard requirement that playback remain at least as smooth and native-resource-stable as the current `wid` baseline. The checked-in desktop fixtures are local-only; no provider stream is involved. The current fixture set is 854x480, so no 1080p CPU result is claimed here. A 1080p H.264 fixture must be included in the prototype before accepting or rejecting a software path on performance grounds.

| Direction | Feasibility | Cost and performance risk | Windows-only fragility | Decision |
| --- | --- | --- | --- | --- |
| **Two top-level windows: `wid` video plus Compose overlay** | Technically plausible. Keep libmpv's existing heavyweight child/video window unchanged; create a borderless transparent overlay window owned by the main window and draw only Tvivo controls in it. This avoids same-window heavyweight ordering and keeps mpv on its already-working presentation path. | Medium implementation cost. No per-frame video copy or new decoder/render path, so playback risk is lowest. The overlay must track the video window's screen coordinates, client insets, DPI, resize, minimize/restore, activation, and cinema-mode bounds. | High. Win32 layered-window styles, DWM composition, focus activation, z-order, transparent hit testing, and multi-monitor DPI all need native verification. A global `alwaysOnTop` overlay is unsafe because it can remain above unrelated applications; prefer an owned/topmost-relative window and hide/deactivate with the player. | **Prototype next.** It is the only route that preserves the proven `wid` baseline while avoiding mpv's render API. It is an explicit exception to the original same-window rule, not a claim that Compose can paint over the Canvas. |
| **mpv `MPV_RENDER_API_TYPE_SW`** | Public and build-independent in principle. `render.h` requires a caller-owned memory surface (`size`, format, stride, pointer) for each render. The buffer can be converted to a Compose `ImageBitmap`/Skia image, but it is CPU compositing rather than a GPU texture path. | High performance risk. mpv documents this renderer as “extremely simple (but slow)”; color conversion, scaling, and OSD/subtitles are CPU-side and single-threaded, and large display sizes can miss realtime. At 1920x1080 RGBA, one output frame alone is about 7.9 MiB before conversion and UI upload; at 60 fps that is roughly 475 MiB/s of pixel writes, a lower-bound bandwidth indication rather than a measurement. The prototype must measure CPU, frame interval/jank, dropped frames, allocation/GC, and working set against `wid`, with double buffering and no per-frame object churn. | Low-to-medium native fragility, but high JVM/Skia upload and scheduling risk. It avoids WGL/D3D interop and is useful as a diagnostic fallback, not as an assumed shippable player. | **Do not prototype first.** Keep as a measured fallback experiment only if the two-window path fails and a 1080p fixture can be tested. Reject unless it meets the same smoothness/resource gate. |
| **D3D11-backed mpv render API / direct Skiko D3D texture** | Not available as a documented public libmpv render API contract in the current `render.h`: its supported-backends section lists OpenGL and software, and its API-type constants expose `"opengl"` and `"sw"`, not D3D11. mpv's D3D11 GPU video backend is not the same thing as a caller-facing render API. The upstream D3D11 render-API request remains an open issue describing a desired future interface for sharing textures/devices, not an implemented ABI. | Potentially excellent if it existed: it could match Skiko's Windows default Direct3D path and avoid GL. Today the cost is an unbounded native fork/build/integration project, with no stable public texture ownership, synchronization, device-loss, or ABI contract to bind. ANGLE can make an OpenGL renderer use a D3D-backed implementation, but it still enters the failing OpenGL render-API initialization path; it does not bypass the observed pre-callback failure. | Very high: device/adapter selection, D3D texture sharing, keyed mutex/fence synchronization, Skiko device ownership, resize/device loss, and build-specific mpv internals. | **Do not pursue for this batch.** Reconsider only if upstream exposes a documented D3D11 render API or a supplied build includes a tested, versioned ABI and native sample. |
| **Other toolkit/player patterns (VLCJ, Qt/QML, JavaFX/native video)** | Existing ecosystems generally solve this by using a toolkit-native video surface/texture or by putting controls in a separate native overlay. mpv's own examples describe `wid` as OS-dependent and the render API as more flexible but OpenGL-based; the Qt/QML examples work because the video is a normal QML/GL element, not because Compose can layer over a heavyweight AWT child. No Kotlin/Compose Desktop pattern found provides a portable, same-window solution for this libmpv/Skiko combination. | Migrating player engines or introducing another UI toolkit violates the current libmpv/native playback decision and adds packaging and lifecycle risk. A VLCJ-style second native window would still reduce to the two-window design already selected for this investigation. | High and outside current scope. | **No separate migration.** Reuse the two-window prototype boundary if a comparable library is consulted for Win32 mechanics. |

**Two-window prototype shape.** The main Compose window remains the owner and retains the current `wid`-embedded video child. A second undecorated top-level window is created only while the player route exists. It is screen-positioned over the video client rectangle, owned by the main window, and raised relative to that owner rather than made globally topmost. Its Compose content contains the top/bottom controls and transparent background. On route exit, deactivation, minimize, or player disposal it is hidden before the `wid` player is released.

The prototype must not assume that `Window(transparent = true)` solves input routing. JetBrains has documented that transparent Compose windows can remain opaque to mouse dispatch, and the Compose/Swing interop issue records that Windows GPU-rendered native views do not alpha-blend reliably in the same window. Win32 layered-window behavior is the relevant contract: `WS_EX_LAYERED` plus `UpdateLayeredWindow`/equivalent can provide per-pixel alpha, while `WS_EX_TRANSPARENT` makes the whole window pass mouse events through. Therefore the overlay needs a native hit-test policy that passes background/video pixels through but keeps visible controls interactive; applying `WS_EX_TRANSPARENT` to the whole overlay is not sufficient. The prototype may use a native `WM_NCHITTEST`/layered-window bridge, but it must remain isolated and documented rather than spread through player code.

**Required evidence before any production controller work:**

- The overlay stays exactly aligned through normal and rapid resize, 100% and 150% DPI, moving between monitors, windowed/cinema bounds, minimize/restore, and repeated show/hide.
- Visible controls receive mouse clicks and keyboard focus; transparent video regions do not block the underlying application/player window. No click-through trick may steal focus from a control or leave an invisible window above another application.
- The overlay never appears above an unrelated foreground application, never flashes during activation, and is destroyed before the `wid` child/player teardown. Five create/show/play/hide/dispose cycles show no retained overlay HWND, Compose window, callback, or native player resource.
- A 60-second local H.264 fixture run is visually as smooth as the current `wid` baseline, with no black frames, resize flash, input loss, sustained private-byte/working-set growth, or duplicate player generation. Record CPU, working set, first-frame time, frame/jank observations, and overlay creation count using the existing baseline procedure.
- Cinema mode remains Compose-owned at the route/state level, but its first implementation is a coordinated main-window/overlay-window geometry transition. It must not ask mpv to enter fullscreen, recreate the player, or reload media.

**Fallback experiment, only if this fails:** add a test-only `MPV_RENDER_API_TYPE_SW` harness using a fixed, aligned, double-buffered `rgb0` surface. Render into the buffer, upload/draw it without allocating a new bitmap per frame, and measure against the `wid` baseline at the actual 1080p fixture size. The result must be reported as measured pass/fail; the public mpv documentation's warning that software rendering may be too slow for realtime is not itself a measured rejection.

Sources consulted: [mpv `render.h` supported backends and software-renderer contract](https://github.com/mpv-player/mpv/blob/master/include/mpv/render.h), [mpv libmpv embedding/render examples](https://github.com/mpv-player/mpv-examples/blob/master/libmpv/README.md), [upstream D3D11 render-API request](https://github.com/mpv-player/mpv/issues/5979), [JetBrains heavyweight Compose/Swing interop limitation](https://github.com/JetBrains/compose-multiplatform/issues/1521), [JetBrains transparent-window input behavior](https://github.com/JetBrains/compose-multiplatform/issues/1401), [Microsoft layered-window hit testing](https://learn.microsoft.com/en-us/windows/win32/winmsg/window-features), and [Java per-pixel translucent-window constraints](https://docs.oracle.com/en/java/javase/15/docs/api/java.desktop/java/awt/Window.html).

#### 0.1.3 Two-window compositing spike result (2026-09-17)

The fixture-only host is `desktop/src/test/kotlin/com/dev/tvivo/desktop/overlay/TwoWindowOverlaySpike.kt`, launched through the test-only `twoWindowOverlaySpike` JavaExec task. It creates a heavyweight AWT `Canvas`, initialises the existing `MpvPlayer` against that Canvas, plays only the caller-supplied local `.mp4`, `.mkv`, or `.ts` fixture, and creates a separate undecorated `ComposeWindow` for three probe controls: Pause/Play, Windowed/Cinema, and Probe click. No production `MpvPlayer.kt` or `DesktopShell.kt` behavior was changed.

The isolated Win32 bridge is `Win32OverlayBridge.kt`. It marks the overlay `WS_EX_LAYERED | WS_EX_TOOLWINDOW`, sets the host HWND as the top-level owner through `GWLP_HWNDPARENT`, installs a retained `WM_NCHITTEST` callback, and restores the original window procedure during disposal. Hit testing returns `HTCLIENT` only for the declared top and bottom control bands and `HTTRANSPARENT` for the video/background region. The overlay is positioned from the Canvas client rectangle in screen coordinates and is refreshed by component events plus a 50 ms coalesced Swing timer. Cinema mode changes only the host/overlay geometry to the current monitor bounds; it does not call mpv fullscreen, recreate the player, or reload the fixture.

The corrected host compiled successfully with `:desktop:testClasses` before the final timestamp/auto-close diagnostic edit. The first launch exposed a JNA callback nullability defect and was stopped; after making all callback pointer arguments nullable and guarding `lParam`, the clean launch used only `sintel-trailer-12s-subtitles.mkv` and reached `Ready`, `file-loaded`, `Playing`, and pause-state callbacks without callback exceptions. The run exceeded 60 seconds. Sampled spike-process metrics were working set **354.3 MiB to 358.3 MiB** and private bytes **391.7 MiB to 394.8 MiB**; these are observations from the overlay run, not a `wid`-only comparison. The log did not include a trustworthy first-frame timestamp before the diagnostic edit.

The acceptance gate is **blocked/incomplete**, not passed. No settled screenshot or direct observation was captured for exact alignment, rapid resize, 100%/150% DPI, monitor crossing, minimize/restore, transparent-region click-through, control click/focus, cinema transition, or unrelated-foreground z-order. The host currently exercises one player/overlay lifecycle only; the five create/show/play/hide/dispose cycles were not run. The clean run was force-stopped after observation, so graceful `MpvPlayer.close()`/overlay callback teardown was not proven; no spike JVM remained afterward. CPU/jank and smoothness were not compared against a separately measured `wid` baseline. The final auto-close/timestamp diagnostic edit was not recompiled before session conclusion. Do not move to production overlay/controller work. The next pass must first compile that final harness, add or execute the five-cycle and input/alignment evidence procedure, and either prove this boundary or reject it with those observations.
